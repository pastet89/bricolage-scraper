...
class QuotesSpider(Spider):
    ...
    :long_, lat: (NoneType) -> longitude and latitude coordinates, later set to string type
    ...
    def __init__(self, cat=None):
        ...
        params:
        :cat: (string) -> Category name with "-" instead of whitespaces """
    ...
    def parse_long_lat(self, response):
        """ Parses the longitude and latitude needed for the store availability processing. """
    ...
    def parse_cats(self, response):
        """ Parses the main categories and after that recursively calls itself with a meta var,
        indicating to parse the subcategories of each main category until the target category is found.
        Once the target category is found, send its link to parse_items_pages() which extracts the pages links
        and the product items.
        """
        ...
        if response.meta.get('cat_type') != "sub_cat":
            ...
            for main_cat in main_cats:
                yield Request(url=main_cat, callback=self.parse_cats, meta={'cat_type':'sub_cat'})
    ...
    def parse_items_pages(self, response):
        """
        1) Extracts the item links from the first page of the target category and sends them for parsing to parse_item()
        2) Extract the next page links of the category from the first page and send them recursively to itself for 
        item link extraction as in 1)
        """
    ...
    def url(self, link):
        ...
        params:
        :link: (string) - The link to which is added the prefix. """
    ...
    def alert(self, msg, is_fatal = True):
        ...
        :msg: (sring) -> Warning message
        :is_fatal: (bool) -> if True, stops the script execution
